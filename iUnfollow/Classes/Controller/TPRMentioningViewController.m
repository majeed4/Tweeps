...
#import "TPRMentioningDetailsViewController.h"

@interface TPRMentioningViewController ()

@end

@implementation TPRMentioningViewController
...
- (void)viewDidLoad
{
    ...
    [super viewDidLoad];
    self.title = NSLocalizedString(@"Mentioning", nil);
...
- (void)updateDataSource
{
    ...
    self.items = [[DataManager sharedInstance] getAllMentioning];
    self.items = [self.items sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"tweetsMentioningUser.@count" ascending:NO]]];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:user.profileImageUrl] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        cell.imageView.image = [UIImage tpr_maskedImageWithImage:image];
...
//        [wcell setNeedsLayout];
//    }];
    cell.retweetsLabel.text = [NSString stringWithFormat:@"%lu", (unsigned long)user.tweetsMentioningUser.count];
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TPRMentioningDetailsViewController *vc = [[TPRMentioningDetailsViewController alloc] initWithUser:user];
